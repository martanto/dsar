...
if __name__ == "__main__":
    ...
    out = r"C:\Users\caudroco\Documents\Papers_2020\WI\DSAR\WSRZ_4_5_8Hz_displ"
    ...
    for file in files:
        ...
        if os.path.isfile(os.path.join(out,filename)) and not force:
            ...
            st.filter("lowpass", freq=8.0)
            stream = DFStream()
            stream+=st
            st=stream
            t = pd.date_range(st[0].stats.starttime.datetime,periods=st[0].stats.npts, freq="%ims"%(st[0].stats.delta*1000))
